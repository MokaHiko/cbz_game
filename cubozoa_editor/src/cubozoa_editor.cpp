...
EditorApplication::EditorApplication(const char *name,
  ...
  sLua["gltf"]["load"] = [&](const char *path) {
    ...
    sGltfs.push_back(GltfAssetCreate(path));

    if (sGltfs[assetId]->load() != cbz::Result::eSuccess) {
      spdlog::error("Failed to load gltf at {}", path);
  ...
  // Laod scripts
  sol::load_result script = sLua.load_file("assets/scripts/init.lua");
  if (!script.valid()) {
...
void EditorApplication::update() {
  ...
  //   if (eId != 16777216) {
  //     sDebugRendererPipeline->focusEntity(eId);
  //   }
  // }
